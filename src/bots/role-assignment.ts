...
export default class RoleAssignmentBot {
    ...
    cheat = ({channel}: Message) => channel.send(`
    ...
    merlin = () => this.players.get(Role.MERLIN)[0].send(
        `Voici les méchants : ${[
    ...
    perceval = () => this.players.get(Role.PERCEVAL)[0].send(
    ...
    lancelot_g = () => this.players.get(Role.LANCELOT_GENTIL)[0].send(
        `Voici ton antagoniste : ${[
            this.players.get(Role.LANCELOT_MECHANT)
        ]
        }`)

    lancelot_m = () => this.players.get(Role.LANCELOT_MECHANT)[0].send(
        `Voici ton antagoniste : ${[
            this.players.get(Role.LANCELOT_GENTIL)
        ]
        }`)

    mechants = (role: Role) => this.players.get(role)[0].send(
    ...
    achieveQuest = (message: Message, quest: Quest) => {
        ...
        if (this.questMembersCount > 0)
            this.mainChannel.send(`Il nous manque encore ${this.questMembersCount} vote(s).`)
